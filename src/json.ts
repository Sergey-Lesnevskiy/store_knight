export const products = [
  {
    id: 1,
    type: 'Защита корпуса',
    title: 'КОЛЬЧУГА',
    description:
      'Реконструкция (стилизация) кольчуги из сведённых колец (края каждого кольца сведены, но не закреплены друг с другом). Кольца круглого сечения, из проволоки толщиной 1,6 мм, внутренний диаметр кольца 8 мм. Плетение 1 в 4. Кольчуга подходит на рост в пределах 172 - 180 см.',
    price: 16160,
    discountPercentage: 0,
    rating: 4.69,
    stock: 3,
    category: 'armor',
    thumbnail:
      'https://donjon.ru/images/1c/9c/9cc17794-0bb7-11e3-a40e-bc5ff462a210_54fdcd97-ec7e-11e7-83b5-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/9c/9cc17794-0bb7-11e3-a40e-bc5ff462a210_54fdcd97-ec7e-11e7-83b5-002590a35117.jpg',
      'https://donjon.ru/images/1c/9c/9cc17794-0bb7-11e3-a40e-bc5ff462a210_14bcabb2-93d1-11e5-97f1-002590a35116.jpg',
    ],
  },
  {
    id: 2,
    type: 'Защита корпуса',
    title: 'КОМПЛЕКТ ДОСПЕХОВ XV ВЕКА ',
    description:
      'Полный комплект европейских рыцарских доспехов середины - второй половины XV века на специальном манекене. Данные доспехи могут быть не только украшением интерьера, но также могут применяться и по прямому назначению. Все элементы доспеха пригодны для ношения.Реконструкция (стилизация) кольчуги из сведённых колец (края каждого кольца сведены, но не закреплены друг с другом). Кольца круглого сечения, из проволоки толщиной 1,6 мм, внутренний диаметр кольца 8 мм. Плетение 1 в 4. Кольчуга подходит на рост в пределах 172 - 180 см.',
    price: 250000,
    discountPercentage: 10,
    rating: 8.69,
    stock: 1,
    category: 'armor',
    thumbnail:
      'https://donjon.ru/images/1c/b9/b9e1a96f-6f24-11ed-8368-002590a35116_3e23870f-70ad-11ed-8368-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/b9/b9e1a96f-6f24-11ed-8368-002590a35116_3e23870f-70ad-11ed-8368-002590a35116.jpg',
      'https://donjon.ru/images/1c/b9/b9e1a96f-6f24-11ed-8368-002590a35116_3e23870a-70ad-11ed-8368-002590a35116.jpg',
    ],
  },
  {
    id: 3,
    type: 'Защита рук',
    title: 'НАЛОКОТНИКИ ТИТАНОВЫЕ',
    description: 'Отдельные стальные налокотники с лепестком. ',
    price: 10750,
    discountPercentage: 2,
    rating: 1.23,
    stock: 100,
    category: 'armor',
    thumbnail:
      'https://donjon.ru/images/1c/cf/cf3b7ab5-5eb0-11e9-872f-002590a35117_068ca3e9-b2b8-11e9-85a6-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/cf/cf3b7ab5-5eb0-11e9-872f-002590a35117_068ca3e9-b2b8-11e9-85a6-002590a35117.jpg',
      'https://donjon.ru/images/1c/cf/cf3b7ab5-5eb0-11e9-872f-002590a35117_068ca3ec-b2b8-11e9-85a6-002590a35117.jpg',
    ],
  },
  {
    id: 4,
    type: 'Защита рук',
    title: 'КУЛАК СТАЛЬНОЙ ТРЕНИРОВОЧНЫЙ',
    description:
      'Стальная защита кисти для спортивных турниров и тренировочных боёв по историческому фехтованию. Исторических аналогов не имеет.',
    price: 6310,
    discountPercentage: 10,
    rating: 1.0,
    stock: 20,
    category: 'armor',
    thumbnail:
      'https://donjon.ru/images/1c/c7/c7023c25-8bd6-11ea-91c8-002590a35117_0e8f3901-8bd7-11ea-91c8-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/c7/c7023c25-8bd6-11ea-91c8-002590a35117_0e8f3901-8bd7-11ea-91c8-002590a35117.jpg',
      'https://donjon.ru/images/1c/c7/c7023c25-8bd6-11ea-91c8-002590a35117_0e8f3903-8bd7-11ea-91c8-002590a35117.jpg',
    ],
  },
  {
    id: 5,
    type: 'Защита головы',
    title: 'БАРМИЦА НА ВОСТОЧНЫЙ ШЛЕМ КЛЁПАНАЯ ',
    description:
      'Полная закрытая кольчужная бармица для восточных и русских шлемов из клёпаных колец. С двумя клиньями по бокам. Закрывает шею спереди. Плетение 4 в 1. Кольца плоские плющенные. Половина колец сплошные (вырубные), половина - клёпаные.',
    price: 17040,
    discountPercentage: 0,
    rating: 6.4,
    stock: 4,
    category: 'armor',
    thumbnail:
      'https://donjon.ru/images/1c/40/4003ac1d-2655-11ea-8458-002590a35117_a8c12f3c-c7c4-11ec-849c-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/40/4003ac1d-2655-11ea-8458-002590a35117_a8c12f3c-c7c4-11ec-849c-002590a35116.jpg',
      'https://donjon.ru/images/1c/40/4003ac1d-2655-11ea-8458-002590a35117_a8c12f3e-c7c4-11ec-849c-002590a35116.jpg',
    ],
  },
  {
    id: 6,
    type: 'Защита ног',
    title: 'НАГОЛЕННИКИ ЛАТНЫЕ ДВУСТВОРЧАТЫЕ',
    description:
      'Наголенники двухстворчатые анатомичные. Стилизация на тему западно-европейской защиты голеней XIV-XV вв.Матовая шлифовка. Марка стали - СТ3. Толщина стали - 1,5 мм.',
    price: 40810,
    discountPercentage: 10,
    rating: 5.4,
    stock: 8,
    category: 'armor',
    thumbnail:
      'https://donjon.ru/images/1c/9c/9c5aeaa9-31ec-11e4-9d7d-002590a35117_3a2a0ed5-64d5-11e8-887b-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/9c/9c5aeaa9-31ec-11e4-9d7d-002590a35117_3a2a0ed5-64d5-11e8-887b-002590a35117.jpg',
      'https://donjon.ru/images/1c/9c/9c5aeaa9-31ec-11e4-9d7d-002590a35117_6afb0731-64d5-11e8-887b-002590a35117.jpg',
    ],
  },
  {
    id: 7,
    type: 'Оружие ближнего боя',
    title: 'АЛЕБАРДА ТИП A',
    description:
      'Алебарда (нем. Hellebarde) — древковое холодное оружие с комбинированным наконечником, состоящим из игольчатого (круглого или гранёного) копейного острия, лезвия боевого топора с одной стороны и, как правило, с крюком или шипом с другой. Находилась на вооружении пехоты ряда европейских стран с XIII по XVII век, получив наибольшее распространение в XV—XVI веках.',
    price: 3200,
    discountPercentage: 20,
    rating: 9.0,
    stock: 1,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/4d/4d74b2bc-de84-11e3-9855-002590a35116_4ecbe23d-40a5-11eb-916f-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/4d/4d74b2bc-de84-11e3-9855-002590a35116_4ecbe23d-40a5-11eb-916f-002590a35116.jpg',
      'https://donjon.ru/images/1c/0d/0db8fc83-7bcb-11e9-872f-002590a35117_07f6abd4-3bac-11ea-9425-002590a35117.jpg',
    ],
  },
  {
    id: 8,
    type: 'Оружие ближнего боя',
    title: 'АЛЕБАРДА ТИП E',
    description:
      'Алебарда (нем. Hellebarde) — древковое холодное оружие с комбинированным наконечником, состоящим из игольчатого (круглого или гранёного) копейного острия, лезвия боевого топора с одной стороны и, как правило, с крюком или шипом с другой. Находилась на вооружении пехоты ряда европейских стран с XIII по XVII век, получив наибольшее распространение в XV—XVI веках.',
    price: 9200,
    discountPercentage: 20,
    rating: 7.0,
    stock: 1,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/db/dbb01fca-5496-11ea-832c-002590a35117_e8b0fb9c-40a4-11eb-916f-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/db/dbb01fca-5496-11ea-832c-002590a35117_e8b0fb9c-40a4-11eb-916f-002590a35116.jpg',
      'https://donjon.ru/images/1c/77/77b94978-de84-11e3-9855-002590a35116_a8d9be09-300c-11e7-8dd6-002590a35116.jpg',
    ],
  },
  {
    id: 9,
    type: 'Стрелковое оружие',
    title: 'АРБАЛЕТ СРЕДНЕВЕКОВЫЙ С КОЗЬЕЙ НОГОЙ',
    description:
      'Арбалет средневековый со стальной дугой. Реконструкция (стилизация) европейского арбалета XV - XVII вв..',
    price: 34080,
    discountPercentage: 10,
    rating: 9.0,
    stock: 5,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/4e/4e0a9e11-2b1d-11e5-98c8-002590a35117_d911e247-e8b2-11e6-8654-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/4e/4e0a9e11-2b1d-11e5-98c8-002590a35117_d911e247-e8b2-11e6-8654-002590a35116.jpg',
      'https://donjon.ru/images/1c/4e/4e0a9e11-2b1d-11e5-98c8-002590a35117_d911e244-e8b2-11e6-8654-002590a35116.jpg',
    ],
  },
  {
    id: 10,
    type: 'Стрелковое оружие',
    title: 'БОЛТ АРБАЛЕТНЫЙ С КОВАНЫМ НАКОНЕЧНИКОМ',
    description:
      'Болт арбалетный с кованым втульчатым наконечником типа бодкин и кожаным оперением. Реконструкция (стилизация).',
    price: 430,
    discountPercentage: 30,
    rating: 2.0,
    stock: 100,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/6f/6f1ea543-2b1c-11e5-98c8-002590a35117_b259818c-2931-11ed-8915-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/6f/6f1ea543-2b1c-11e5-98c8-002590a35117_b259818c-2931-11ed-8915-002590a35116.jpg',
      'https://donjon.ru/images/1c/6f/6f1ea543-2b1c-11e5-98c8-002590a35117_b259818d-2931-11ed-8915-002590a35116.jpg',
    ],
  },
  {
    id: 11,
    type: 'Оружие ближнего боя',
    title: 'КИНЖАЛ БАСЕЛАРД',
    description:
      'Реплика средневекового кинжала баселард. Баселард  — тип кинжала и короткого меча, распространенного в Европе с первой половины XIII до конца XV века.',
    price: 8000,
    discountPercentage: 20,
    rating: 5.0,
    stock: 5,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/96/9635098e-0bb7-11e3-a40e-bc5ff462a210_3cdf8a50-bb81-11e6-8654-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/96/9635098e-0bb7-11e3-a40e-bc5ff462a210_3cdf8a50-bb81-11e6-8654-002590a35116.jpg',
      'https://donjon.ru/images/1c/96/9635098e-0bb7-11e3-a40e-bc5ff462a210_3cdf8a51-bb81-11e6-8654-002590a35116.jpg',
    ],
  },
  {
    id: 12,
    type: 'Оружие ближнего боя',
    title: 'КИНЖАЛ РОНДЕЛ ИЗ ЛИДСА ',
    description:
      'Реплика европейского рондельного кинжала. Конкретный артефакт, взятый за образец - кинжал из Англии, датированный началом XV века, и хранящийся он в Королевском Арсенале',
    price: 7200,
    discountPercentage: 20,
    rating: 5.0,
    stock: 5,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/96/96350992-0bb7-11e3-a40e-bc5ff462a210_0892544c-6102-11e9-872f-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/96/96350992-0bb7-11e3-a40e-bc5ff462a210_0892544c-6102-11e9-872f-002590a35117.jpg',
      'https://donjon.ru/images/1c/96/96350992-0bb7-11e3-a40e-bc5ff462a210_0892544b-6102-11e9-872f-002590a35117.jpg',
    ],
  },
  {
    id: 13,
    type: 'Стрелковое оружие',
    title: 'АРБАЛЕТНЫЙ КОЛЧАН ',
    description:
      'Классический средневековый европейский колчан для арбалетных стрел (болтов). Колчаны такого типа не только встречаются на средневековых изобразительных источниках, но и, начиная с XIV века известны также и в виде сохравнившихся экземпляров.',
    price: 17200,
    discountPercentage: 10,
    rating: 2.0,
    stock: 3,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/39/3932f684-460c-11ec-818a-002590a35116_5a3a328f-2930-11ed-8915-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/39/3932f684-460c-11ec-818a-002590a35116_5a3a328f-2930-11ed-8915-002590a35116.jpg',
      'https://donjon.ru/images/1c/39/3932f684-460c-11ec-818a-002590a35116_5a3a328d-2930-11ed-8915-002590a35116.jpg',
    ],
  },
  {
    id: 14,
    type: 'Оружие ближнего боя',
    title: 'КАРОЛИНГ ТРЕНИРОВОЧНЫЙ ТИП X',
    description:
      'Меч Каролинг стальной тренировочный тип X. Спортивная стилизация на средневековые европейские мечи каролингского типа (VIII-XI века) для ИСБ и турниров по раннесредневековому фехтованию.',
    price: 17200,
    discountPercentage: 10,
    rating: 2.0,
    stock: 3,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/8c/8c5f175e-8754-11ec-81b2-002590a35116_8a9e2f3a-8756-11ec-81b2-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/8c/8c5f175e-8754-11ec-81b2-002590a35116_8a9e2f3a-8756-11ec-81b2-002590a35116.jpg',
      'https://donjon.ru/images/1c/96/963509b0-0bb7-11e3-a40e-bc5ff462a210_c04f2087-62d6-11ed-8025-002590a35116.jpg',
    ],
  },
  {
    id: 15,
    type: 'Оружие ближнего боя',
    title: 'КАРОЛИНГ ТУРНИРНЫЙ ТИП Z ОСОБЫЙ',
    description:
      'Стальной полновесный макет каролингского меча тип Z особый. Предназначен для турниров по раннесредневековому фехтованию и ИСБ.',
    price: 6700,
    discountPercentage: 10,
    rating: 8.0,
    stock: 4,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/4e/4e3dc7fd-e021-11ec-809d-002590a35116_572040d1-17de-11ed-8374-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/4e/4e3dc7fd-e021-11ec-809d-002590a35116_572040d1-17de-11ed-8374-002590a35116.jpg',
      'https://donjon.ru/images/1c/4e/4e3dc7fd-e021-11ec-809d-002590a35116_572040d2-17de-11ed-8374-002590a35116.jpg',
    ],
  },
  {
    id: 16,
    type: 'Оружие ближнего боя',
    title: 'КЛЕЙМОР СТАЛЬНОЙ СПОРТИВНЫЙ',
    description:
      'Стальной полновесный макет каролингского меча тип Z особый. Предназначен для турниров по раннесредневековому фехтованию и ИСБ.',
    price: 7080,
    discountPercentage: 10,
    rating: 8.0,
    stock: 2,
    category: 'weapon',
    thumbnail:
      'https://donjon.ru/images/1c/96/963509b1-0bb7-11e3-a40e-bc5ff462a210_10b8af36-1de7-11eb-90b1-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/96/963509b1-0bb7-11e3-a40e-bc5ff462a210_10b8af36-1de7-11eb-90b1-002590a35117.jpg',
      'https://donjon.ru/images/1c/96/963509b1-0bb7-11e3-a40e-bc5ff462a210_10b8af37-1de7-11eb-90b1-002590a35117.jpg',
    ],
  },
  {
    id: 17,
    type: 'Галантерея',
    title: 'ПЕРЕВЯЗЬ ДЛЯ МЕЧА',
    description:
      'Простая перевязь для ношения оружия (меча, сабли и т.п.) на поясе. Материал - натуральная кожа. Стальные кованые пряжки.',
    price: 3010,
    discountPercentage: 10,
    rating: 4.0,
    stock: 4,
    category: 'costumes',
    thumbnail:
      'https://donjon.ru/images/1c/9c/9cc1767f-0bb7-11e3-a40e-bc5ff462a210_f7c73545-b2b9-11e9-85a6-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/9c/9cc1767f-0bb7-11e3-a40e-bc5ff462a210_f7c73545-b2b9-11e9-85a6-002590a35117.jpg',
      'https://donjon.ru/images/1c/9c/9cc1767f-0bb7-11e3-a40e-bc5ff462a210_5705103b-6101-11e9-872f-002590a35117.jpg',
    ],
  },
  {
    id: 18,
    type: 'Галантерея',
    title: 'ПУГОВИЦА O-020 РУСЬ',
    description: 'Пуговица. Русь. XII-XV вв. Литье. Материал - латунь.',
    price: 190,
    discountPercentage: 0,
    rating: 1.0,
    stock: 29,
    category: 'costumes',
    thumbnail:
      'https://donjon.ru/images/1c/b2/b2501eff-0bb7-11e3-a40e-bc5ff462a210_ba2f30d5-2939-11e9-90d7-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/b2/b2501eff-0bb7-11e3-a40e-bc5ff462a210_ba2f30d5-2939-11e9-90d7-002590a35117.jpg',
      'https://donjon.ru/images/1c/b2/b2501eff-0bb7-11e3-a40e-bc5ff462a210_ba2f30d6-2939-11e9-90d7-002590a35117.jpg',
    ],
  },
  {
    id: 19,
    type: 'Верхняя одежда',
    title: 'КОТТА МУЖСКАЯ ',
    description:
      'Реконструкция (стилизация) европейской средневековой мужской рубахи котты на основе находки «Бокстейнский болотный человек» (Швеция, 2-я половина XIV века), а также различных изобразительных источников XI - XIV вв. Упрощённый вариант',
    price: 9500,
    discountPercentage: 0,
    rating: 8.0,
    stock: 7,
    category: 'costumes',
    thumbnail:
      'https://donjon.ru/images/1c/a5/a525b8ef-0bb7-11e3-a40e-bc5ff462a210_2ee77126-ec03-11e7-83b5-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/a5/a525b8ef-0bb7-11e3-a40e-bc5ff462a210_2ee77126-ec03-11e7-83b5-002590a35117.jpg',
      'https://donjon.ru/images/1c/a5/a525b8ef-0bb7-11e3-a40e-bc5ff462a210_2ee77127-ec03-11e7-83b5-002590a35117.jpg',
    ],
  },
  {
    id: 20,
    type: 'Верхняя одежда',
    title: 'СРЕДНЕВЕКОВЫЙ КАПЮШОН ШАППЕРОН',
    description:
      'Средневековый капюшон (худ или шапперон). Западная, Центральная, Южная и Северная Европа, XIII - XV вв. Реконструкция (стилизация). Упрощённый вариант (машинный шов).',
    price: 3200,
    discountPercentage: 10,
    rating: 6.0,
    stock: 4,
    category: 'costumes',
    thumbnail:
      'https://donjon.ru/images/1c/a5/a525b8f8-0bb7-11e3-a40e-bc5ff462a210_eda13521-ec02-11e7-83b5-002590a35117.jpg',
    images: [
      'https://donjon.ru/images/1c/a5/a525b8f8-0bb7-11e3-a40e-bc5ff462a210_eda13521-ec02-11e7-83b5-002590a35117.jpg',
      'https://donjon.ru/images/1c/a5/a525b8f8-0bb7-11e3-a40e-bc5ff462a210_eda13524-ec02-11e7-83b5-002590a35117.jpg',
    ],
  },

  {
    id: 21,
    type: 'Обувь',
    title: 'БОТИНКИ СРЕДНЕВЕКОВЫЕ X-XV ВЕКОВ ',
    description:
      'Исторический макет средневековых европейских ботинок X-XV века. Предназначены для исторической реконструкции, ролевых игр живого действия (LARP) и театров. Реконструкция (стилизация) на основе различных находок из Западной Европы.',
    price: 4500,
    discountPercentage: 8,
    rating: 6.0,
    stock: 3,
    category: 'costumes',
    thumbnail:
      'https://donjon.ru/images/1c/5d/5d87e126-086a-11ea-8458-002590a35117_a8d16aa7-3fd8-11eb-8180-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/5d/5d87e126-086a-11ea-8458-002590a35117_a8d16aa7-3fd8-11eb-8180-002590a35116.jpg',
      'https://donjon.ru/images/1c/5d/5d87e126-086a-11ea-8458-002590a35117_a8d16aa8-3fd8-11eb-8180-002590a35116.jpg',
    ],
  },
  {
    id: 22,
    type: 'Галантерея',
    title: 'СУМОЧКА ПОЯСНАЯ БАРАТЕОН',
    description:
      'Сумочка поясная с гербом дома Баратеонов. Изготовлена по мотивам серии книг и сериала Джорджа Р. Р. Мартина Песнь Льда и Пламени. Выполнена из плотной натуральной кожи, богато украшена латунью.',
    price: 3800,
    discountPercentage: 0,
    rating: 9.0,
    stock: 1,
    category: 'costumes',
    thumbnail:
      'https://donjon.ru/images/1c/a3/a384719d-c2da-11e6-8654-002590a35116_ef019ec9-cea7-11e6-8654-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/a3/a384719d-c2da-11e6-8654-002590a35116_ef019ec9-cea7-11e6-8654-002590a35116.jpg',
      'https://donjon.ru/images/1c/a3/a384719d-c2da-11e6-8654-002590a35116_ef019eca-cea7-11e6-8654-002590a35116.jpg',
    ],
  },
  {
    id: 23,
    type: 'Головные уборы',
    title: 'БРАСЛЕТ SUV-193 ВОЛКИ СРЕДНИЙ ',
    description:
      'Браслет в скандинавском стиле с изображением волчьих голов. Литье. Материал - латунь. Длина внутренней окружности в сомкнутом виде - около 17,5 см.',
    price: 1800,
    discountPercentage: 0,
    rating: 4.0,
    stock: 2,
    category: 'jewelry',
    thumbnail:
      'https://donjon.ru/images/1c/b2/b2adec16-58fe-11ec-8c93-002590a35116_d913a583-c67e-11ec-849c-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/b2/b2adec16-58fe-11ec-8c93-002590a35116_d913a583-c67e-11ec-849c-002590a35116.jpg',
      'https://donjon.ru/images/1c/b2/b2adec16-58fe-11ec-8c93-002590a35116_d913a584-c67e-11ec-849c-002590a35116.jpg',
    ],
  },
  {
    id: 24,
    type: 'Головные уборы',
    title: 'КОРОНА СРЕДНЕВЕКОВАЯ',
    description:
      'Стилизация на тему средневековых европейских корон. Верх выполнен в виде французских лилии. Украшена вставками из красных кабошонов. Материал - латунь. Рассчитана на голову с обхватом около 56 см.',
    price: 31700,
    discountPercentage: 0,
    rating: 4.0,
    stock: 1,
    category: 'jewelry',
    thumbnail:
      'https://donjon.ru/images/1c/ff/ff6e461b-e9b9-11e9-8392-002590a35117_5dec22cb-78e0-11ed-8368-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/ff/ff6e461b-e9b9-11e9-8392-002590a35117_5dec22cb-78e0-11ed-8368-002590a35116.jpg',
      'https://donjon.ru/images/1c/ff/ff6e461b-e9b9-11e9-8392-002590a35117_5dec22c9-78e0-11ed-8368-002590a35116.jpg',
    ],
  },
  {
    id: 25,
    type: 'Серьги',
    title: 'СЕРЬГИ O-108 ВИЗАНТИЯ ',
    description: 'Серьги. Византия. X-XII век. Литье. Материал - латунь. Цена указана за пару.',
    price: 2250,
    discountPercentage: 0,
    rating: 4.0,
    stock: 5,
    category: 'jewelry',
    thumbnail:
      'https://donjon.ru/images/1c/b2/b2501f14-0bb7-11e3-a40e-bc5ff462a210_28042077-0172-11ed-85f1-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/b2/b2501f14-0bb7-11e3-a40e-bc5ff462a210_28042077-0172-11ed-85f1-002590a35116.jpg',
      'https://donjon.ru/images/1c/b2/b2501f14-0bb7-11e3-a40e-bc5ff462a210_28042079-0172-11ed-85f1-002590a35116.jpg',
    ],
  },
  {
    id: 26,
    type: 'Кольца',
    title: 'КОЛЬЦО R-032',
    description: 'Кольцо витое. Русь, Европа, Прибалтика, Скандинавия X-XIII век. Литье. Материал: латунь',
    price: 510,
    discountPercentage: 0,
    rating: 4.0,
    stock: 2,
    category: 'jewelry',
    thumbnail:
      'https://donjon.ru/images/1c/b2/b2501f8d-0bb7-11e3-a40e-bc5ff462a210_398de386-d42f-11ec-809d-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/b2/b2501f8d-0bb7-11e3-a40e-bc5ff462a210_398de386-d42f-11ec-809d-002590a35116.jpg',
      'https://donjon.ru/images/1c/b2/b2501f8d-0bb7-11e3-a40e-bc5ff462a210_398de387-d42f-11ec-809d-002590a35116.jpg',
    ],
  },
  {
    id: 27,
    type: 'Посуда',
    title: 'КУБОК ЛАНДСКНЕХТ',
    description:
      'Кубок. Реконструкция (стилизация). Западная Европа. Ландскнехты, Швейцария, XVI- XVII века. Материал: латунь, посеребрение.',
    price: 8000,
    discountPercentage: 10,
    rating: 6.0,
    stock: 20,
    category: 'houseHold',
    thumbnail:
      'https://donjon.ru/images/1c/ab/ab3a86ae-7954-11ed-8368-002590a35116_f9a69c75-7954-11ed-8368-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/ab/ab3a86ae-7954-11ed-8368-002590a35116_f9a69c75-7954-11ed-8368-002590a35116.jpg',
      'https://donjon.ru/images/1c/ab/ab3a86ae-7954-11ed-8368-002590a35116_f9a69c76-7954-11ed-8368-002590a35116.jpg',
    ],
  },
  {
    id: 28,
    type: 'Инструменты',
    title: 'СРЕДНЕВЕКОВЫЕ КОВАНЫЕ НОЖНИЦЫ (АНТ)',
    description: 'Средневековые стальные кованные ножницы.',
    price: 1600,
    discountPercentage: 0,
    rating: 4.0,
    stock: 6,
    category: 'houseHold',
    thumbnail:
      'https://donjon.ru/images/1c/cb/cb2e328b-0434-11e4-919f-002590a35117_f1c25708-c61c-11ec-8381-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/cb/cb2e328b-0434-11e4-919f-002590a35117_f1c25708-c61c-11ec-8381-002590a35116.jpg',
      'https://donjon.ru/images/1c/cb/cb2e328b-0434-11e4-919f-002590a35117_f1c25705-c61c-11ec-8381-002590a35116.jpg',
    ],
  },
  {
    id: 29,
    type: 'Прочее',
    title: 'ПЛЁТКА ШЛЯХТИЧ (БМВ)',
    description:
      'Простая плеть. Реконструкция по различным изображениям XV-XVII вв. Подобная простая плеть в частности изображена на раскрашенном рельефе XVII века Танец смерти из польского городка Tarłów.',
    price: 7650,
    discountPercentage: 0,
    rating: 7.0,
    stock: 2,
    category: 'houseHold',
    thumbnail:
      'https://donjon.ru/images/1c/22/225134d6-7e5b-11e6-8e79-002590a35116_747a33cb-7e5b-11e6-8e79-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/22/225134d6-7e5b-11e6-8e79-002590a35116_747a33cb-7e5b-11e6-8e79-002590a35116.jpg',
      'https://donjon.ru/images/1c/22/225134d6-7e5b-11e6-8e79-002590a35116_e5fdd2dc-7e5b-11e6-8e79-002590a35116.jpg',
    ],
  },
  {
    id: 30,
    type: 'Прочее',
    title: 'СРЕДНЯЯ ВОСКОВАЯ СВЕЧА (ЧП)',
    description: 'Восковая свеча. Вес - около 450 гр. Высота - около 10 см. Диаметр - около 8 см.',
    price: 240,
    discountPercentage: 0,
    rating: 5.0,
    stock: 28,
    category: 'houseHold',
    thumbnail:
      'https://donjon.ru/images/1c/37/376cf490-2a79-11eb-8e88-005056c00008_ce662690-3fd9-11eb-8180-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/37/376cf490-2a79-11eb-8e88-005056c00008_ce662690-3fd9-11eb-8180-002590a35116.jpg',
      'https://donjon.ru/images/1c/37/376cf490-2a79-11eb-8e88-005056c00008_ce66268f-3fd9-11eb-8180-002590a35116.jpg',
    ],
  },
  {
    id: 31,
    type: 'Укрытия',
    title: 'ШАТЁР КРУГЛЫЙ «ГРЕБЕЛЬЕР» БОЛЬШОЙ Б/У (ОМ)',
    description:
      'Шатер круглый большой, бывший в употреблении. Имеет следы загрязнения. Вход распашной на завязках, стены отъемные на завязках. В комплектацию входят 2 отдельных чехла: для стен и для купола, колышки, растяжки (пеньковая, джутовая). Стены имеют петли под колышки, контактные места прикрепления и опоры усилены дупликатурами ткани.',
    price: 40000,
    discountPercentage: 12,
    rating: 7.0,
    stock: 5,
    category: 'houseHold',
    thumbnail:
      'https://donjon.ru/images/1c/98/982a133d-c5ce-11e4-9476-002590a35117_44fbecfb-83cb-11e5-a371-002590a35116.jpg',
    images: [
      'https://donjon.ru/images/1c/98/982a133d-c5ce-11e4-9476-002590a35117_44fbecfb-83cb-11e5-a371-002590a35116.jpg',
      'https://donjon.ru/images/1c/98/982a133d-c5ce-11e4-9476-002590a35117_52bc8032-83cb-11e5-a371-002590a35116.jpg',
    ],
  },
];
